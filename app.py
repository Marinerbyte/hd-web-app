...
class BotState:
    def __init__(self):
        ...
        # --- NEW: Gunicorn-safe lifecycle management ---
        self.bot_thread = None
        self.stop_bot_signal = threading.Event() # To signal the bot manager to stop the bot
        self.start_bot_signal = threading.Event() # To signal the bot manager to start the bot
...
# ========================================================================================
# === DATABASE & BACKGROUND TASKS (Unchanged) ============================================
# ========================================================================================
...
def cleanup_old_logs():
    ...
    while True: # This thread runs for the app's lifetime
        try:
            if supabase:
            ...
            time.sleep(Config.LOG_CLEANUP_INTERVAL_SECONDS)
        except Exception as e:
            logging.error(f"[DB] Error in log cleanup thread: {e}", exc_info=True)
            time.sleep(60)

# ========================================================================================
# === ROAMER & CYCLE LOGIC (Unchanged) ===================================================
# ========================================================================================
...
# ========================================================================================
# === WEB APP & BOT LIFECYCLE (HEAVILY MODIFIED FOR GUNICORN) ============================
# ========================================================================================
app = Flask(__name__)
...
<body class="flex h-screen overflow-hidden">
    <aside id="sidebar" class="sidebar flex flex-col w-64 p-4 space-y-6">
        ...
        <div class="flex items-center justify-between">
             <a href="#" onclick="controlBot('start')" class="flex-1 mr-1 text-center bg-green-500/80 hover:bg-green-500 text-white font-bold py-2 px-4 rounded transition-colors duration-300"><i class="fas fa-play"></i></a>
             <a href="#" onclick="controlBot('stop')" class="flex-1 ml-1 text-center bg-red-500/80 hover:bg-red-500 text-white font-bold py-2 px-4 rounded transition-colors duration-300"><i class="fas fa-stop"></i></a>
        </div>
        ...
        <div id="chat-container" class="chat-area flex-1 flex flex-col rounded-xl border border-gray-700 overflow-hidden">
            ...
            <div id="chat-input-container" class="p-4 border-t border-gray-600">
                <form id="message-form" class="flex items-center space-x-3">
                    ...
                    <button type="submit" id="send-button" disabled class="bg-cyan-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-cyan-400 disabled:bg-gray-500 disabled:cursor-not-allowed transition-colors">
...
function renderMessage(activity) {
    ...
    if (type === 'message') {
        ...
        bubbleHtml = `<div class="flex flex-col ${isBot ? 'items-end' : 'items-start'}"><div class="text-xs text-gray-400 mb-1">${username} <span class="font-fira-code text-gray-500">${time}</span></div><div class="${bubbleClass} max-w-lg p-3 rounded-xl shadow"><p class="text-white">${text.replace(/</g, "&lt;").replace(/>/g, "&gt;")}</p></div></div>`;
    } else if (type === 'system') {
        bubbleHtml = `<div class="chat-bubble-system text-center text-sm py-1"><span class="font-fira-code text-gray-500">${time}</span> - ${text.replace(/</g, "&lt;").replace(/>/g, "&gt;")}</div>`;
    }
    return bubbleHtml;
}
async function fetchAndUpdateStatus() {
    try {
        ...
        if (data.is_running) {
            statusOrb.style.setProperty('--color', data.is_connected ? '#22c55e' : '#f97316');
            statusText.textContent = data.is_connected ? 'Running & Connected' : 'Running & Disconnected';
        } else {
            statusOrb.style.setProperty('--color', '#ef4444');
            statusText.textContent = 'Stopped';
        }
        statusOrb.className = 'status-orb w-4 h-4 rounded-full transition-all duration-500';
...
async function sendMessage(event) {
    ...
    try {
        ...
        input.value = '';
        const tempActivity = { type: 'message', username: '{{ bot_name }}', text: text, timestamp: new Date().toISOString() };
        const chatWindow = document.getElementById('chat-window');
...
def start_bot_route():
    if not session.get('logged_in'):
        # Allow uptime key even if not logged in
        if not (request.args.get('key') and request.args.get('key') == Config.UPTIME_SECRET_KEY):
            return "Not authorized", 401
    
    logging.info("WEB PANEL: Received request to start the bot.")
    bot_state.start_bot_signal.set()
...
def stop_bot_route():
    ...
    bot_state.start_bot_signal.clear()
    return redirect(url_for('home'))
...
def send_ws_message(payload):
    if bot_state.is_connected and bot_state.ws_instance:
        try: bot_state.ws_instance.send(json.dumps(payload))
        except Exception as e: logging.error(f"Error sending message: {e}")
    else: logging.warning("Warning: WebSocket is not connected.")
def reply_to_room(room_id, text): send_ws_message({"handler": "chatroommessage", "type": "text", "roomid": room_id, "text": text})
...
def join_room(room_name, source=None): send_ws_message({"handler": "joinchatroom", "name": room_name, "roomPassword": "", "__source": source})
def join_startup_rooms():
    logging.info("Joining startup rooms...")
    time.sleep(1)
    if not (rooms_str := Config.ROOMS_TO_JOIN):
        logging.info("No startup rooms defined in ROOMS_TO_JOIN."); return
    for room_name in [name.strip() for name in rooms_str.split(',')]:
        if bot_state.stop_bot_signal.is_set(): break
        if room_name: time.sleep(Config.ROOM_JOIN_DELAY_SECONDS); join_room(room_name, source='startup_join')
    if not bot_state.stop_bot_signal.is_set(): logging.info("✅ Finished joining startup rooms.")
...
def on_open(ws):
    ...
    bot_state.is_connected = True; bot_state.reconnect_delay = Config.INITIAL_RECONNECT_DELAY
    send_ws_message({"handler": "login", "username": Config.BOT_USERNAME, "password": Config.BOT_PASSWORD, "token": bot_state.token})
    for room_name in bot_state.startup_rooms: add_activity(room_name, {'type': 'system', 'text': 'WebSocket connected.'})
        ...
        if handler == "login" and data.get("status") == "success":
            ...
            with bot_state.roam_lock:
                for room in data["data"]:
                    if "name" in room and room.get("userCount", 0) > 0: bot_state.roamable_rooms.add(room["name"])
...
def on_error(ws, error):
    ...
    for room_name in bot_state.startup_rooms: add_activity(room_name, {'type': 'system', 'text': f'WebSocket Error: {error}'})
def on_close(ws, close_status_code, close_msg):
    bot_state.is_connected = False
    for room_name in bot_state.startup_rooms: add_activity(room_name, {'type': 'system', 'text': 'WebSocket connection closed.'})
    if bot_state.stop_bot_signal.is_set(): logging.info("--- Bot gracefully stopped by panel. ---")
    else: logging.warning(f"--- WebSocket closed unexpectedly. Reconnecting in {bot_state.reconnect_delay}s... ---")
...
def connect_to_howdies():
    ...
    logging.info("Bot's connect_to_howdies loop has ended.")
...
# ========================================================================================
# === BOT MANAGER THREAD FOR GUNICORN-SAFE OPERATION =====================================
# ========================================================================================
def bot_manager():
    """
    This function runs in a background thread and manages the bot's lifecycle.
    It waits for start/stop signals from the Flask routes.
    """
    logging.info("🤖 Bot Manager thread started.")
    while True:
        # Wait for the start signal
        bot_state.start_bot_signal.wait()

        # Check if bot should be running and isn't already
        if bot_state.bot_should_be_running and (not bot_state.bot_thread or not bot_state.bot_thread.is_alive()):
            logging.info("Bot Manager: Start signal received. Starting bot thread.")
            
            # Reset activity logs and state
            with bot_state.activity_lock:
            ...
            # Create and start the main bot thread
            bot_state.bot_thread = threading.Thread(target=connect_to_howdies, daemon=True)
            bot_state.bot_thread.start()
        
        # Now, wait for the stop signal while the bot runs
        bot_state.stop_bot_signal.wait()
        ...
        if bot_state.bot_thread and bot_state.bot_thread.is_alive():
            logging.info("Bot Manager: Stop signal received. Waiting for bot thread to join.")
            bot_state.bot_thread.join(timeout=5)
        
        bot_state.bot_thread = None
        bot_state.bot_should_be_running = False
        with bot_state.activity_lock:
            for room_name in bot_state.startup_rooms:
                 add_activity(room_name, {'type': 'system', 'text': 'Bot stopped.'})
        
        # Clear signals for the next cycle
        bot_state.start_bot_signal.clear()
        bot_state.stop_bot_signal.clear()
        logging.info("Bot Manager: Bot is fully stopped. Awaiting next start signal.")

@app.before_request
def before_first_request_func():
    # A bit of a hack to start threads only once in a Gunicorn environment.
    # The 'gunicorn.sock' check is to ensure this runs only in the master process.
    if 'gunicorn.sock' in os.environ.get('GUNICORN_SOCKETS', ''):
        return
        
    # Check if the manager is already running to avoid duplicates
    if not any(t.name == 'BotManager' for t in threading.enumerate()):
        # Start the bot manager thread
        manager_thread = threading.Thread(target=bot_manager, name="BotManager", daemon=True)
        manager_thread.start()

        # Start the log cleanup thread
        log_cleanup_thread = threading.Thread(target=cleanup_old_logs, name="LogCleanup", daemon=True)
...
if __name__ == "__main__":
    ...
    # Start background threads when running locally (not with Gunicorn)
    if not any(t.name == 'BotManager' for t in threading.enumerate()):
        manager_thread = threading.Thread(target=bot_manager, name="BotManager", daemon=True)
        manager_thread.start()
        log_cleanup_thread = threading.Thread(target=cleanup_old_logs, name="LogCleanup", daemon=True)
    ...
    # When running with Gunicorn, start the background tasks here
    # This block will be executed by the master Gunicorn process
    if not any(t.name == 'BotManager' for t in threading.enumerate()):
        manager_thread = threading.Thread(target=bot_manager, name="BotManager", daemon=True)
        manager_thread.start()
        log_cleanup_thread = threading.Thread(target=cleanup_old_logs, name="LogCleanup", daemon=True)
