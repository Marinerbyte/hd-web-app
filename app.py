...
class Config:
    ...
    LOG_CLEANUP_INTERVAL_SECONDS = 60 * 60 # Cleanup logs every hour
...
class BotState:
    def __init__(self):
        ...
        # --- UPDATED --- visited_roam_rooms will now be loaded from/synced with Supabase
        ...
        # --- REMOVED --- roam_log is now fully in Supabase
...
# --- NEW --- Function to load visited rooms from Supabase on startup
...
# --- NEW --- Background thread to cleanup old logs from Supabase
    ...
    while not bot_state.stop_bot_event.is_set():
        try:
            if supabase:
                ...
                # logging.info(f"[DB] Deleting logs older than {expire_time.isoformat()}")
            ...
            logging.error(f"[DB] Error in log cleanup thread: {e}")
            bot_state.stop_bot_event.wait(60) # Wait a minute on error
...
def perform_roam_action(target_room):
    # ... (No changes here, except DB interaction)
    ...
    with bot_state.roam_lock:
        ...
        if supabase:
            try:
                # Add to visited rooms table
                supabase.table('visited_rooms').upsert({
                    'room_name': target_room,
                    'visited_at': current_time_iso
                }).execute()
                # Add to log table
                supabase.table('roam_logs').insert({
                    'room_name': target_room,
                    'prize_won': prize_won,
                    'roam_time': current_time_iso
                }).execute()
            except Exception as e:
                logging.error(f"[DB] Error syncing roam data: {e}")
...
def roamer_logic():
    ...
    while not bot_state.stop_roamer_event.is_set():
        try:
            ...
            with bot_state.roam_lock:
                now = time.time()
                # --- UPDATED --- Cleanup expired rooms from memory AND database
                ...
                for room_name in expired_rooms:
                    ...
                    if supabase:
                        try:
                            ...
                            logging.error(f"[DB] Failed to delete expired room '{room_name}': {e}")
...
def send_pm(user_id, text): send_ws_message({"handler": "pm", "userid": user_id, "text": text})
...
def handle_help(room_id):
    ...
    help_text = (
        ...
        "-----------------------------------\n"
        "**General:** `!j <room>`\n"
        "**Master-Only:** `!quiz on|off`, `!delay [min] [max]`, `!cycle on|off|status`\n"
        "**Roamer:** `!roamer on|off|status`, `!roamlog`, `!roamnow <room>`\n"
        "**Global (PM only):** `!statusall`"
    )
    reply_to_room(room_id, help_text)
def handle_roamnow_command(args, room_id):
    # ... (Unchanged)
    if not args:
        reply_to_room(room_id, "Usage: `!roamnow <room_name>`")
        return
    target_room = " ".join(args)
    reply_to_room(room_id, f"✅ Roger that! Forcing a roam to '{target_room}'. Check PMs for results.")
...
# --- UPDATED --- !roamlog now reads from Supabase
def handle_roamlog_command(room_id):
    if not supabase:
        return reply_to_room(room_id, "❌ Database not configured. Cannot fetch roam logs.")
    try:
        response = supabase.table('roam_logs').select("room_name, prize_won, roam_time").order("roam_time", desc=True).limit(10).execute()
        
        if not response.data:
            reply_to_room(room_id, "No spin activity recorded.")
            return
        ...
        logging.error(f"[DB] Error fetching roam log: {e}")
        reply_to_room(room_id, "❌ Error fetching logs from the database.")

# --- UPDATED --- !quiz on/off now controls cycle mode
def handle_quiz_command(sub_command, args, room_id):
    if sub_command == 'on':
        ...
        reply_to_room(room_id, "Usage: `!quiz on <bot>` or `!quiz off`")
def handle_delay_command(args, room_id):
    ...
    if not args:
        ...
        return
    try:
        if len(args) != 2: reply_to_room(room_id, "Usage: `!delay <min_ms> <max_ms>`"); return
...
def handle_cycle_command(sub_command, room_id):
    ...
    if sub_command == 'on':
        ...
        else: stop_cycle_for_room(room_id, show_message=True); reply_to_room(room_id, "✅ Cycle mode deactivated.")
    elif sub_command == 'status':
        if room_id in bot_state.cycle_timers:
            now = time.time()
            if (break_end := bot_state.break_end_times.get(room_id)) and now < break_end:
                ends_in = break_end - now
                ...
                ends_in_min = (work_end - now) / 60
                reply_to_room(room_id, f"✳️ Cycle ON: Working. Next break in {ends_in_min:.1f} minutes.")
            else:
                 reply_to_room(room_id, "✳️ Cycle ON: State is transitioning...")
...
def on_message(ws, message_str):
    # ... (Unchanged, PM logic is already correct)
    ...
    try:
        ...
        if handler == "login" and data.get("status") == "success":
            ...
            load_visited_rooms_from_db() # --- NEW --- Load state from DB
            ...
            sender_id, sender_name, text = data.get('userid'), data.get('username', '').lower(), data.get('text', '').strip()
            if sender_name in bot_state.masters and text == '!statusall':
                logging.info(f"PM from master '{sender_name}': Received !statusall command.")
                status_lines = ["🤖 **ArcadeBot Global Status** 🤖"]
                roamer_status = "ON" if bot_state.is_roamer_active else "OFF"
                status_lines.append(f"\n--- Roamer ---\n• Status: **{roamer_status}**")
                for room_id, room_name in bot_state.room_id_to_name.items():
                    status_lines.append(f"\n--- Room: **{room_name}** ---")
                    if (quiz_bot := bot_state.quiz_solvers.get(room_id)): status_lines.append(f"• Quiz Solver: **ON** (for bot *{quiz_bot}*)")
