...
if Config.SUPABASE_URL and Config.SUPABASE_KEY:
    ...
    logging.warning("⚠️ Supabase credentials not found. Roam logs will be in-memory only.")
...
class BotState:
    def __init__(self):
        ...
        self.stop_bot_signal = threading.Event()
...
def cleanup_old_logs():
    ...
    while not bot_state.stop_bot_signal.is_set():
        try:
            if supabase:
            ...
            bot_state.stop_bot_signal.wait(Config.LOG_CLEANUP_INTERVAL_SECONDS)
        except Exception as e:
            logging.error(f"[DB] Error in log cleanup thread: {e}", exc_info=True)
            bot_state.stop_bot_signal.wait(60)
...
def schedule_next_break(room_id):
    if room_id not in bot_state.cycle_timers or bot_state.stop_bot_signal.is_set(): return
...
<head>
    ...
    <style> body { background-color: #111827; } .login-box { background-color: rgba(31, 41, 55, 0.8); backdrop-filter: blur(10px); } </style>
</head>
        ...
        {% with messages = get_flashed_messages(with_categories=true) %}
            {% if messages %}
                {% for category, message in messages %}
                    <div class="p-4 text-sm text-red-200 bg-red-800/50 rounded-lg" role="alert">{{ message }}</div>
                {% endfor %}
            {% endif %}
        {% endwith %}
        <form method="post" class="space-y-6">
            <div><input type="text" name="username" placeholder="Username" required class="w-full px-4 py-2 text-white bg-gray-900 border border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-cyan-500"></div>
            <div><input type="password" name="password" placeholder="Password" required class="w-full px-4 py-2 text-white bg-gray-900 border border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-cyan-500"></div>
            <button type="submit" class="w-full px-4 py-2 font-bold text-gray-900 bg-cyan-400 rounded-md hover:bg-cyan-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-cyan-500 transition-colors duration-300">Login</button>
        </form>
    </div>
</body>
</html>
...
<head>
    ...
    <style>
        ...
        .font-fira-code { font-family: 'Fira Code', monospace; } .sidebar { background-color: rgba(31, 41, 55, 0.5); backdrop-filter: blur(12px); border-right: 1px solid rgba(255,255,255,0.1); }
        .chat-area { background-color: rgba(17, 24, 39, 0.7); backdrop-filter: blur(12px); } .chat-bubble-user { background-color: #4b5563; }
        .chat-bubble-bot { background-color: #3b82f6; } .chat-bubble-system { color: #9ca3af; font-style: italic; }
        .status-orb { box-shadow: 0 0 10px var(--color), 0 0 20px var(--color); } .nav-item.active { background-color: var(--accent-blue); color: white; }
        .nav-item:not(.active):hover { background-color: #4b5563; } ::-webkit-scrollbar { width: 8px; } ::-webkit-scrollbar-track { background: transparent; }
        ::-webkit-scrollbar-thumb { background-color: #4b5563; border-radius: 4px; } ::-webkit-scrollbar-thumb:hover { background-color: #6b7280; }
    </style>
</head>
<body class="flex h-screen overflow-hidden">
    <aside id="sidebar" class="sidebar flex flex-col w-64 p-4 space-y-6">
        <div class="flex items-center space-x-3"><div id="status-orb" class="status-orb w-4 h-4 rounded-full transition-all duration-500"></div><h1 class="text-xl font-bold">{{ bot_name }}</h1></div>
        <div><p class="text-sm text-gray-400">Status</p><p id="status-text" class="font-medium">Initializing...</p></div>
        ...
        </div> <hr class="border-gray-600">
        <div class="flex-grow overflow-y-auto"><h2 class="text-sm font-semibold text-gray-400 mb-2">Joined Rooms</h2><nav id="room-list" class="space-y-1"></nav></div>
        <hr class="border-gray-600"> <a href="/logout" class="w-full text-center bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded transition-colors duration-300"><i class="fas fa-sign-out-alt mr-2"></i>Logout</a>
    </aside>
    <main class="flex-1 flex flex-col p-4">
        <div id="chat-container" class="chat-area flex-1 flex flex-col rounded-xl border border-gray-700 overflow-hidden">
            <div id="chat-header" class="p-4 border-b border-gray-600 text-lg font-semibold">Select a room to view activity</div>
            ...
            <div id="chat-input-container" class="p-4 border-t border-gray-600">
                <form id="message-form" class="flex items-center space-x-3">
                    <input type="text" id="message-input" placeholder="Type a message or command..." disabled class="flex-1 bg-gray-900 px-4 py-2 rounded-lg border border-gray-600 focus:outline-none focus:ring-2 focus:ring-cyan-500 transition-all">
                    <button type="submit" id="send-button" disabled class="bg-cyan-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-cyan-400 disabled:bg-gray-500 disabled:cursor-not-allowed transition-colors"><i class="fas fa-paper-plane"></i></button>
                </form>
            </div>
        </div>
    </main>
<script>
let currentRoom = null; let activityCache = {};
function escapeHtml(text) { return String(text).replace(/</g, "&lt;").replace(/>/g, "&gt;"); }
function formatTimestamp(isoString) { const date = new Date(isoString); return date.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }); }
function renderMessage(activity) {
    const { type, username, text, timestamp } = activity;
    const time = formatTimestamp(timestamp); let bubbleHtml = '';
    if (type === 'message') {
        const isBot = username === '{{ bot_name }}'; const bubbleClass = isBot ? 'chat-bubble-bot self-end' : 'chat-bubble-user self-start';
        bubbleHtml = `<div class="flex flex-col ${isBot ? 'items-end' : 'items-start'}"><div class="text-xs text-gray-400 mb-1">${escapeHtml(username)} <span class="font-fira-code text-gray-500">${time}</span></div><div class="${bubbleClass} max-w-lg p-3 rounded-xl shadow"><p class="text-white">${escapeHtml(text)}</p></div></div>`;
    } else if (type === 'system') {
        bubbleHtml = `<div class="chat-bubble-system text-center text-sm py-1"><span class="font-fira-code text-gray-500">${time}</span> - ${escapeHtml(text)}</div>`;
    } return bubbleHtml;
}
async function fetchAndUpdateStatus() {
    try {
        const response = await fetch('/api/status');
        if (!response.ok) { document.getElementById('status-text').textContent = 'API Error'; return; }
        const data = await response.json(); const statusOrb = document.getElementById('status-orb'); const statusText = document.getElementById('status-text');
        if (data.is_running) { statusOrb.style.setProperty('--color', data.is_connected ? '#22c55e' : '#f97316'); statusText.textContent = data.is_connected ? 'Running & Connected' : 'Running & Disconnected';
        } else { statusOrb.style.setProperty('--color', '#ef4444'); statusText.textContent = 'Stopped'; }
        const roomList = document.getElementById('room-list'); let roomListHtml = '';
        data.joined_rooms.forEach(room => {
            const isActive = room.name.toLowerCase() === currentRoom;
            roomListHtml += \`<a href="#" class="nav-item flex items-center p-2 rounded-md transition-colors duration-200 \${isActive ? 'active' : ''}" onclick="selectRoom('${room.name.toLowerCase()}')"><i class="fas fa-hashtag w-5 text-gray-400"></i><span>\${room.name}</span></a>\`;
        }); roomList.innerHTML = roomListHtml;
    } catch (error) { console.error('Error fetching status:', error); document.getElementById('status-text').textContent = 'Fetch Failed'; }
}
async function fetchAndUpdateActivity(roomName) {
    if (!roomName) return;
    try {
        const response = await fetch(\`/api/activity/\${roomName}\`); if (!response.ok) return; const data = await response.json(); const newActivities = data.activities;
        if (JSON.stringify(activityCache[roomName]) === JSON.stringify(newActivities)) return;
        activityCache[roomName] = newActivities; const chatWindow = document.getElementById('chat-window');
...
function selectRoom(roomName) {
    currentRoom = roomName.toLowerCase(); document.getElementById('chat-header').textContent = \`Activity in #\${roomName}\`;
    document.querySelectorAll('.nav-item').forEach(el => { el.classList.remove('active'); if (el.innerText.toLowerCase().trim() === roomName) { el.classList.add('active'); } });
    document.getElementById('message-input').disabled = false; document.getElementById('send-button').disabled = false;
    document.getElementById('message-input').placeholder = \`Message #\${roomName}\`; document.getElementById('chat-window').innerHTML = '';
    activityCache[roomName] = null; fetchAndUpdateActivity(currentRoom);
}
async function sendMessage(event) {
    event.preventDefault(); const input = document.getElementById('message-input'); const text = input.value.trim();
    if (!text || !currentRoom) return;
    try {
        await fetch('/api/send_message', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ room_name: currentRoom, text: text }) });
        input.value = ''; const tempActivity = { type: 'message', username: '{{ bot_name }}', text: text, timestamp: new Date().toISOString() };
        const chatWindow = document.getElementById('chat-window'); chatWindow.insertAdjacentHTML('beforeend', renderMessage(tempActivity));
        chatWindow.scrollTop = chatWindow.scrollHeight;
    } catch (error) { console.error('Error sending message:', error); }
}
document.addEventListener('DOMContentLoaded', () => { fetchAndUpdateStatus(); setInterval(fetchAndUpdateStatus, 3000); setInterval(() => fetchAndUpdateActivity(currentRoom), 2000); document.getElementById('message-form').addEventListener('submit', sendMessage); });
</script>
</body>
</html>
"""
...
def start_bot_route():
    is_uptime_call = request.args.get('key') and request.args.get('key') == Config.UPTIME_SECRET_KEY
    if not session.get('logged_in') and not is_uptime_call:
        return "Not authorized", 401
    if bot_state.bot_thread and bot_state.bot_thread.is_alive():
        logging.warning("Start command received, but bot is already running.")
    else:
        logging.info("Start command received. Starting bot in a new thread.")
        bot_state.stop_bot_signal.clear()
        with bot_state.activity_lock:
        ...
        bot_state.bot_thread = threading.Thread(target=connect_to_howdies, daemon=True)
        bot_state.bot_thread.start()
    if is_uptime_call:
        return "Bot start initiated."
    return redirect(url_for('home'))
...
def stop_bot_route():
    if not session.get('logged_in'):
        return "Not authorized", 401
    logging.info("Stop command received. Signaling bot to stop.")
    bot_state.stop_bot_signal.set()
    ...
    if bot_state.ws_instance:
        ...
        except Exception as e: logging.error(f"Error closing WebSocket on stop: {e}")
    with bot_state.activity_lock:
        for room_name in bot_state.startup_rooms:
            add_activity(room_name, {'type': 'system', 'text': 'Stop signal sent. Bot is shutting down.'})
    return redirect(url_for('home'))
...
def api_status():
    ...
    is_running = bot_state.bot_thread and bot_state.bot_thread.is_alive()
    if not is_running:
...
def api_send_message():
    if not session.get('logged_in'): return jsonify({"error": "Not authenticated"}), 401
    data = request.json
    room_name, text = data.get('room_name'), data.get('text')
    if not room_name or not text: return jsonify({"error": "Missing room_name or text"}), 400
    room_id = bot_state.room_name_to_id.get(room_name.lower())
    if not room_id: return jsonify({"error": "Bot is not in that room"}), 404
...
def send_delayed_quiz_answer(room_id, answer_text):
    if bot_state.stop_bot_signal.is_set(): return
    delay_ms = random.randint(Config.QUIZ_ANSWER_DELAY_MIN_MS, Config.QUIZ_ANSWER_DELAY_MAX_MS)
    time.sleep(delay_ms / 1000.0)
    if not bot_state.stop_bot_signal.is_set():
        reply_to_room(room_id, answer_text)
        if room_name := bot_state.room_id_to_name.get(room_id):
...
def handle_status_command(room_id):
    ...
    if (quiz_bot := bot_state.quiz_solvers.get(room_id)): status_lines.append(f"• Quiz Solver: **ON** (for *{quiz_bot}*)")
    else: status_lines.append("• Quiz Solver: **OFF**")
    if room_id in bot_state.cycle_timers:
        now = time.time()
        if (break_end := bot_state.break_end_times.get(room_id)) and now < break_end: status_lines.append(f"• Cycle Mode: **ON** (On Break for {break_end - now:.0f}s)")
        elif (work_end := bot_state.work_end_times.get(room_id)) and now < work_end: status_lines.append(f"• Cycle Mode: **ON** (Working for {(work_end - now) / 60:.1f} more mins)")
...
# ========================================================================================
# === WEBSOCKET HANDLERS & CONNECTION LOGIC ==============================================
# ========================================================================================
def connect_to_howdies():
    logging.info("connect_to_howdies thread started.")
    bot_state.token = get_token()
    if not bot_state.token or bot_state.stop_bot_signal.is_set():
        logging.error("Could not get token or stop signal was set before starting.")
        bot_state.is_connected = False
        return
    ...
    while not bot_state.stop_bot_signal.is_set():
        ws_app.run_forever(ping_interval=30, ping_timeout=10, sslopt={"cert_reqs": ssl.CERT_NONE})
        if bot_state.stop_bot_signal.is_set():
            break
        logging.info(f"WebSocket connection ended. Will try to reconnect in {bot_state.reconnect_delay}s.")
        bot_state.is_connected = False
    ...
    if bot_state.bot_thread is threading.current_thread():
        bot_state.bot_thread = None
    logging.info("connect_to_howdies thread has finished.")
...
def on_open(ws):
    ...
    # Start background cleanup thread only when bot is running
...
def join_startup_rooms():
    ...
    for room_name in [name.strip() for name in rooms_str.split(',')]:
        if bot_state.stop_bot_signal.is_set(): break
        if room_name:
...
def on_message(ws, message_str):
    ...
    try:
        ...
        if handler == "login" and data.get("status") == "success":
            ...
            for room_name in bot_state.startup_rooms: add_activity(room_name, {'type': 'system', 'text': f'Logged in as {Config.BOT_USERNAME}.'})
            ...
            if room_name := bot_state.room_id_to_name.get(room_id): add_activity(room_name, {'type': 'system', 'text': f'Kicked from room.'})
            ...
            if room_name := bot_state.room_id_to_name.pop(room_id, None):
                ...
                if room_name.lower() in bot_state.startup_rooms:
                    logging.warning(f"⚠️ Kicked from startup room '{room_name}'. Rejoining..."); time.sleep(Config.REJOIN_ON_KICK_DELAY_SECONDS)
                    join_room(room_name, source='startup_join')
                else: logging.warning(f"⚠️ Kicked from '{room_name}'. Not a startup room.")
        elif handler == "chatroommessage":
            room_id = data.get('roomid')
            if not (room_name := bot_state.room_id_to_name.get(room_id)): return
            text, user_id, username = data.get('text', '').strip(), data.get('userid'), data.get('username')
            ...
            if room_id == bot_state.listening_for_prize_in_room:
                if (prize := extract_prize(text, Config.BOT_USERNAME)) and bot_state.prize_found_event:
                    bot_state.last_prize_won = prize; bot_state.prize_found_event.set()
            if str(user_id) == str(bot_state.bot_user_id): return
            if text.startswith('!'): threading.Thread(target=process_command, args=({'id': user_id, 'name': username}, room_id, text), daemon=True).start()
            if room_id in bot_state.quiz_solvers: threading.Thread(target=process_quiz_message, args=(room_id, text, username), daemon=True).start()
...
def on_close(ws, close_status_code, close_msg):
    ...
    if not bot_state.stop_bot_signal.is_set():
        for room_name in bot_state.startup_rooms:
            add_activity(room_name, {'type': 'system', 'text': 'WebSocket connection closed unexpectedly.'})
    else:
        logging.info("WebSocket closed due to stop signal.")
...
if __name__ == "__main__":
    logging.info(f"--- Starting Web Panel for {Config.BOT_USERNAME} (Local Development) ---")
