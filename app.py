...
def schedule_next_break(room_id):
    if room_id not in bot_state.cycle_timers or bot_state.stop_bot_event.is_set():
        return
...
def stop_cycle_for_room(room_id):
    ...
    # <--- MODIFICATION: This line is now commented out and will not run --->
    # reply_to_room(room_id, "✅ Cycle mode deactivated. The bot will now answer all questions.")
    # <-------------------------------------------------------------------->

# ========================================================================================
# === WEB APP (Omitted for brevity, it's unchanged) ======================================
# ========================================================================================
app = Flask(__name__)
...
# ========================================================================================
# === CORE BOT UTILITIES & COMMANDS (Mostly Unchanged) ===================================
# ========================================================================================
def load_masters():
...
def send_ws_message(payload):
    if bot_state.is_connected and bot_state.ws_instance:
        try:
            if payload.get("text") not in Config.CYCLE_START_COMMANDS and payload.get("text") not in Config.CYCLE_STOP_COMMANDS:
...
def handle_help(room_id):
    help_text = (
        ...
        "-----------------------------------\n"
        "**General:** `!j <room>`\n"
        "**Master-Only:** `!quiz on <bot>`, `!quiz off`, `!delay [min] [max]`, `!cycle on|off|status`"
    )
...
def handle_delay_command(args, room_id):
    ...
    try:
        ...
        if new_min < 0 or new_max < 0:
            reply_to_room(room_id, "❌ Error: Delay values cannot be negative.")
            return
        if new_min > new_max:
            reply_to_room(room_id, "❌ Error: Minimum delay cannot be greater than maximum delay.")
            return
        ...
        reply_to_room(room_id, "❌ Error: Please provide valid numbers for delay values.")

def handle_cycle_command(sub_command, room_id):
    if sub_command == 'on':
        if room_id not in bot_state.quiz_solvers:
            reply_to_room(room_id, "ℹ️ Please turn the quiz solver on first with `!quiz on <bot>`.")
        elif room_id in bot_state.cycle_timers:
            reply_to_room(room_id, "ℹ️ Cycle mode is already active in this room.")
        else:
            start_cycle_for_room(room_id)
    elif sub_command == 'off':
        if room_id not in bot_state.cycle_timers:
            reply_to_room(room_id, "ℹ️ Cycle mode is not active in this room.")
        else:
            stop_cycle_for_room(room_id)
        ...
        if room_id in bot_state.cycle_timers:
            if time.time() < bot_state.break_end_times.get(room_id, 0):
                ...
                reply_to_room(room_id, "✳️ Cycle mode is ON and the bot is currently working (answering questions).")
        else:
            reply_to_room(room_id, "⚪ Cycle mode is OFF.")
    else:
        reply_to_room(room_id, "Usage: `!cycle on`, `!cycle off`, or `!cycle status`")
...
# ========================================================================================
# === QUIZ SOLVER, WEBSOCKETS & MAIN (Mostly Unchanged) ==================================
# ========================================================================================
