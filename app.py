...
class Config:
    ...
    MASTERS_LIST = os.getenv("MASTERS_LIST", "yasin") # Masters from .env
    ...
    # These are now the default/initial values. They can be changed at runtime.
...
def handle_help(room_id):
    # <--- MODIFICATION: Added !delay command to help text --->
    help_text = (
        "🤖 **ArcadeBot Help Menu** 🤖\n"
        "-----------------------------------\n"
        "**General:** `!j <room>`\n"
        "**Master-Only:** `!quiz on <bot>`, `!quiz off`, `!delay [min_ms] [max_ms]`"
    )
...
# <--- MODIFICATION: New function to handle the !delay command --->
def handle_delay_command(args, room_id):
    if not args:
        # No arguments, just show the current settings
        min_delay = Config.QUIZ_ANSWER_DELAY_MIN_MS
        max_delay = Config.QUIZ_ANSWER_DELAY_MAX_MS
    ...
    try:
        ...
        # Update the config values at runtime
        Config.QUIZ_ANSWER_DELAY_MIN_MS = new_min
        Config.QUIZ_ANSWER_DELAY_MAX_MS = new_max
...
def process_command(sender, room_id, message_text):
    ...
    if command == 'help': handle_help(room_id)
        ...
        if command == 'quiz':
            handle_quiz_command(args[0] if args else '', args[1:], room_id)
        # <--- MODIFICATION: Add the new delay command handler --->
        elif command == 'delay':
            handle_delay_command(args, room_id)
        # <-------------------------------------------------------->
...
# ========================================================================================
setup_logging()
load_masters() # MOVED HERE TO RUN ON SERVER STARTUP
